...
def train(_epsilon, _epsilon_decay, _min_epsilon, _alpha, _gamma, _q_network, _episodes, _optimizer):
    ...
    for episode in range(_episodes):
        ...
        while not done:
            x = np.asarray(to_one_hot(state, 16), dtype=np.float32)
            ...
            if done:
                ...
                reward = -0.1
                tensor_next_state = torch.tensor(np.asarray(to_one_hot(next_state, 16), dtype=np.float32))
